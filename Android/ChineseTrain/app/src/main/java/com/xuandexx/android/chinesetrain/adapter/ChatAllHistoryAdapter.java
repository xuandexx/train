...
public class ChatAllHistoryAdapter extends ArrayAdapter<EMConversation> {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (isGroup) {
			...
			if (username.equals(ComConfig.GROUP_USERNAME)) {
				holder.name.setText("群聊");

			}
			if (username.equals(ComConfig.FIND_NEW_FRIENDS)) {
				holder.name.setText("寻找新朋友");

			} else if (username.equals(ComConfig.NEW_FRIENDS_USERNAME)) {
				holder.name.setText("新朋友验证请求");
			}
	...
	private String getMessageDigest(EMMessage message, Context context) {
		...
		case TXT: // 文本消息
			if (!message.getBooleanAttribute(
					ComConfig.MESSAGE_ATTR_IS_VOICE_CALL, false)) {
