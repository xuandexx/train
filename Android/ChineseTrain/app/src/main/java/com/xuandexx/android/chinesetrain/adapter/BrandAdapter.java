 ...
 * 
 */
public class BrandAdapter extends MyActivity implements OnWheelChangedListener {
		...
		submit_address.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (null == mAreaDatasMap.get(mCurrentCityName)) {
					...
					intent.putExtra("town",
							mAreaDatasMap.get(mCurrentCityName)[current]);
				}
				setResult(1, intent);
				finish();
			}
	...
	private void initJsonData() {
		try {
			...
			InputStream is = getAssets().open("carbrand.json");
			int len = -1;
	...
	private void initDatas() {
		try {
			...
			JSONArray jsonArray = mJsonObj.getJSONArray("carbrandlist");
